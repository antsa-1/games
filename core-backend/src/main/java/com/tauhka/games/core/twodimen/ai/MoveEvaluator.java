...
public class MoveEvaluator {
	...
	public static Move calculateMove(TicTacToeTable table, List<Move> availableMoves) {
		...
		Move bestMove = null;
		Move centerPoint = new Move((int) Math.ceil(table.getBoard().length / 2), (int) Math.ceil(table.getBoard().length / 2));
	...
	private static void checkHorizontals(GameToken[][] board, Move move, int requiredAmount, GameToken followedToken, Move centerPoint) {
		...
		double leftSideEval = (totalDirectConnections + directConnectionsOnLeftSide) * CONNECTING_VALUE_WEIGHT.doubleValue() - (Math.abs((startingPointX - centerPoint.getX()) * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()))
				- (Math.abs(startingPointY - centerPoint.getY() * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()));
		double rightSideEval = (totalDirectConnections + directConnectionsOnRightSide) * CONNECTING_VALUE_WEIGHT.doubleValue() - (Math.abs((startingPointX - centerPoint.getX()) * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()))
				- (Math.abs((startingPointY - centerPoint.getY()) * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()));
	...
	private static void checkVerticals(GameToken[][] board, Move move, int requiredAmount, GameToken followedToken, Move centerPoint) {
		int startingPointX = move.getX();
		int startingPointY = move.getY();
		GameToken oppositeToken = followedToken == GameToken.X ? GameToken.O : GameToken.X;
		int totalDirectConnections = 0;
		...
		double upEval = (totalDirectConnections + directConnectionsUp) * CONNECTING_VALUE_WEIGHT.doubleValue() - (Math.abs((startingPointX - centerPoint.getX()) * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()))
				- (Math.abs(startingPointY - centerPoint.getY() * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()));
		double downEval = (totalDirectConnections + directConnectionsDown) * CONNECTING_VALUE_WEIGHT.doubleValue() - (Math.abs((startingPointX - centerPoint.getX()) * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()))
				- (Math.abs((startingPointY - centerPoint.getY()) * DISTANCE_FROM_CENTER_WEIGHT.doubleValue()));
